...
interface Doc { name: string; size: string; date: string; type: string }
...
export default function Documents() {
  ...
  const handleUpload = (files: FileList | null) => {
    ...
    setDocs(prev => [{ name: file.name, size, date: new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }), type: 'Uploaded' }, ...prev])
    setToast(`${file.name} uploaded`)
    setTimeout(() => setToast(null), 2500)
  }

  const remove = (name: string) => setDocs(prev => prev.filter(d => d.name !== name))

  return (
    <div className="page-enter">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.03em' }}>Documents</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: 2 }}>
            {isAdvocate ? 'Client documents & case files' : 'FIRs, contracts, receipts, and AI reports'}
          </p>
        </div>
        <div>
          <input ref={fileRef} type="file" style={{ display: 'none' }} onChange={e => handleUpload(e.target.files)} />
          <button onClick={() => fileRef.current?.click()} className="btn-primary"
            style={{ padding: '9px 16px', borderRadius: 9, fontSize: '0.85rem', fontWeight: 600, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {toast && (
        <div style={{
          marginBottom: 16, padding: '10px 14px', borderRadius: 8, background: 'var(--emerald-subtle)',
      ...
      <div
        ...
        className="card"
        style={{ padding: 28, marginBottom: 20, textAlign: 'center', border: '2px dashed var(--border)', cursor: 'pointer' }}
        onClick={() => fileRef.current?.click()}
      >
        <Upload size={26} style={{ color: 'var(--text-subtle)', marginBottom: 8 }} />
        <div style={{ fontWeight: 600, color: 'var(--text)', fontSize: '0.9rem' }}>Drag & drop a file, or click to browse</div>
        <div style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: 3 }}>PDF, DOCX, JPG up to 20MB</div>
      </div>
      ...
      <div className="card" style={{ padding: 8 }}>
        {docs.map((d, i) => (
          <div key={i} style={{
            ...
            <span className="badge" style={{ background: 'var(--blue-subtle)', color: 'var(--blue)', flexShrink: 0 }}>{d.type}</span>
            <button title="Download" style={{
              padding: 7, borderRadius: 6, border: '1px solid var(--border)',
              background: 'var(--bg-card)', color: 'var(--text-muted)', cursor: 'pointer',
              display: 'flex', flexShrink: 0,
            }}><Download size={13} /></button>
            <button title="Remove" onClick={() => remove(d.name)} style={{
              padding: 7, borderRadius: 6, border: '1px solid var(--border)',
